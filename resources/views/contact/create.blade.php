...
@section('content')
  ...
  <section id="contacto" class="py-16">
    <div class="container mx-auto px-4 grid md:grid-cols-2 gap-8 items-stretch min-h-[35rem]">
      ...
      {{-- Columna izquierda: imagen con overlay y texto --}}
      <div class="relative rounded-xl overflow-hidden h-full">
        <img src="{{ asset('images/libros-ingleses.jpg') }}" alt="Comunicación y asesoramiento en inglés y traducción"
      ...
      <div class="h-full">
        <div class="bg-azul rounded-xl shadow p-6 md:p-8 h-full flex flex-col">
          ...
          <div class="bg-azul overflow-hidden shadow-sm sm:rounded-lg pt-4 pb-6 px-6 flex-1 text-white">
            ...
            <form method="POST" action="{{ route('contact.store') }}" class="space-y-4 h-full flex flex-col"
              ...
              <div>
                ...
                <input id="name" name="name" class="w-full border rounded p-2 bg-white text-azul"
              ...
              <div>
                ...
                <input id="email" type="email" name="email" class="w-full border rounded p-2 bg-white text-azul"
              ...
              <div>
                ...
                <textarea id="message" name="message" rows="5" class="w-full border rounded p-2 bg-white text-azul"
                  @if(request('subject'))
                  placeholder="Cuéntame tu nivel, objetivos y horarios preferidos para agendar la clase de prueba." @endif
                  required>{{ old('message') }}</textarea>
              </div>

              {{-- Checkbox aceptación GDPR --}}
